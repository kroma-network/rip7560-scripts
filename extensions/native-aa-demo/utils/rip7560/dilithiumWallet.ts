...
import { publicClient, bundlerClient } from "utils/chain/client";
import { 
  decodeFunctionData, 
  encodeFunctionData,
  keccak256,
...
import { nonceManagerPredeployAddress } from "viem-rip7560/src/experimental/rip7560/accounts/address";
  ...
  return toNativeSmartAccount({
    ...
    async getNonce(parameters) {
      const res = await publicClient.call({
        to: nonceManagerPredeployAddress,
        data: encodePacked(['address', 'uint192'], [address, parameters?.key ?? BigInt(0)]),
      })
      ...
      const lower64BitMask = BigInt('0xffffffffffffffff');
      const lower64Bits = BigInt(res.data) & lower64BitMask;
      const nonce = lower64Bits.toString(16).padStart(16, '0') as Hex;

      return hexToNumber(nonce ?? '0x0')
    },
